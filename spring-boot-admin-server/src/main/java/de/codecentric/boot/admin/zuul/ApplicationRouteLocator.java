...
public class ApplicationRouteLocator extends ProxyRouteLocator {
	...
	public ApplicationRouteLocator(String servletPath,
			ApplicationRegistry registry, ZuulProperties properties,
			String prefix) {
		...
		if (registry != null) {
			for (Application application : registry.getApplications()) {
				addRoute(locateRoutes, prefix + "/" + application.getId()
						+ "/health/**", application.getHealthUrl());
	...
	private void addRoute(LinkedHashMap<String, ZuulRoute> locateRoutes,
