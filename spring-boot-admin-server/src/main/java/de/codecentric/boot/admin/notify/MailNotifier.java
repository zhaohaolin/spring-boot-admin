...
public class MailNotifier implements
		ApplicationListener<ClientApplicationStatusChangedEvent> {
	...
	public MailNotifier(MailSender sender) {
		this.sender = sender;
		this.subject = parser.parseExpression(DEFAULT_SUBJECT,
				ParserContext.TEMPLATE_EXPRESSION);
		this.text = parser.parseExpression(DEFAULT_TEXT,
				ParserContext.TEMPLATE_EXPRESSION);
	}
	...
	public void onApplicationEvent(ClientApplicationStatusChangedEvent event) {
		if (enabled
				&& shouldSendMail(event.getFrom().getStatus(), event.getTo()
						.getStatus())) {
			try {
				sendMail(event);
			} catch (Exception ex) {
				LOGGER.error("Couldn't send mail for Statuschange {} ", event,
						ex);
			}
		}
	}
	
	private void sendMail(ClientApplicationStatusChangedEvent event)
	...
	public void setSubject(String subject) {
		this.subject = parser.parseExpression(subject,
				ParserContext.TEMPLATE_EXPRESSION);
	}
	
	public void setText(String text) {
		this.text = parser.parseExpression(text,
				ParserContext.TEMPLATE_EXPRESSION);
	}
